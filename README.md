...
> In this template, we use `@selectio` as a placeholder for package names. Replace it with your own organization or project name using find-and-replace to change all instances of `@selectio` to something like `@my-company` or `@project-name`.
...
bun --filter @selectio/auth generate
```
...
3. Outputs a Drizzle-compatible schema file to the `@selectio/db` package
